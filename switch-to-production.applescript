...
# Optional parameters:
# @raycast.icon 🌐
# @raycast.description Switches current Chrome tab from local development server to production
...
if frontApp is not "Google Chrome" then
    my showMessage("Please switch to Chrome first", "Switch to Production", debugMode)
    return
end if

tell application "Google Chrome"
    if (count of windows) = 0 then
        my showMessage("No Chrome windows open", "Switch to Production", debugMode)
        return
    end if
    ...
    -- Check if the URL contains the local URL
    if currentURL does not contain localURL then
        my showMessage("Current page is not on " & localURL, "Switch to Production", debugMode)
        return
    end if

    -- Simple find and replace: local URL → production URL
    try
        set newURL to my replaceText(currentURL, localURL, productionURL)

        if debugMode then
            my showMessage("About to navigate to: " & newURL, "Debug Info", debugMode)
        end if
        ...
        if debugMode then
            my showMessage("Switched to production server: " & newURL, "Switch to Production", debugMode)
        else
            my showMessage("Switched to production server", "Switch to Production", debugMode)
        end if
